...
journey takes the customer by its identifier, runs inside the house dialog on the page
(an iframe on Salt Edge Connect, with a link to a tab for a browser that will not frame
it), and comes back to `/app/banque/retour` on the deployment's own origin, whose domain
must be among the app's allowed redirect domains in the Salt Edge dashboard; that page
hands the bank screen's address to the frame's parent (or a tab's opener), which moves
there and closes the dialog; the provider's class of refusal reaches the button and
