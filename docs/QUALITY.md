...
| --- | --- | --- |
| Typecheck, lint, unit and integration tests, production build | `.github/workflows/ci.yml`, job **checks** | The code compiles, follows the rules (including `react/no-unstable-nested-components`), the 303 vitest tests pass, `next build` succeeds. Each is its own named step, so a red run says which one. |
