...
| End-to-end, real browser, real backend | job **e2e**, step *End-to-end tests* | The built app (`next start`) against a throwaway local Supabase: sign-up, sign-in, sign-out, two accounts on one browser, Safari-style autofill, property creation, rental dossier to activation, tenant invitation accepted from the link by a new account, a tenant request with a photo answered, tracked, turned into an intervention and resolved from the owner's Messages (read back in the tenant's space at each step), cross-account isolation (a foreign workspace and a tenant account answered 404 and 403 on the request routes), departure, the former tenant's history, and the typing checks (a field keeps the caret). Chromium desktop for everything, WebKit in an iPhone 14 viewport for the door and the typing checks. |
